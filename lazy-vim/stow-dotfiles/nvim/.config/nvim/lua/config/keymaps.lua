...
-- 「終了コマンドの:qを間違えてq:と打ってしまうとコマンドラインウィンドウが開いてしまう」と説明しましたが、
-- 以下の設定で無効化できます。
vim.keymap.set("n", "q:", "<nop>", { desc = "Disable cmdwin" })

-- ノーマルモードで <A-j> (Alt+j) を20行下に移動するコマンドにマッピング
vim.keymap.set("n", "<A-j>", "20j", { noremap = true, silent = true })
-- ノーマルモードで <A-j> (Alt+j) を20行下に移動するコマンドにマッピング
vim.keymap.set("n", "<A-k>", "20k", { noremap = true, silent = true })
